...
const FeedbackModal = ({ 
  ...
  const starBg = useColorModeValue('gray.100', 'gray.700');
  const starHoverBg = useColorModeValue('gray.200', 'gray.600');
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} initialFocusRef={initialRef} isCentered>
      <ModalOverlay />
      <ModalContent borderRadius="md" maxW="450px" bg={modalBg}>
        <ModalHeader textAlign="center" fontSize="2xl" fontWeight="bold" color={headerColor}>Your Feedback</ModalHeader>
        <ModalCloseButton />
        <ModalBody pb={6} color={bodyTextColor}>
          <Text textAlign="center" color={bodyTextColor} mb={6}>
            ...
            <HStack spacing={2} justify="center">
              {[1, 2, 3, 4, 5].map((star) => (
                <Box
                  ...
                  borderRadius="md"
                  bg={starBg}
                  _hover={{ bg: starHoverBg }}
                >
                  <StarIcon
                    w={6}
                    h={6}
                    color={(hoveredRating || rating) >= star ? 'orange.400' : useColorModeValue('gray.300', 'gray.500')}
                  />
                </Box>
              ))}
            </HStack>
