-- Fix athlete_workouts RLS policies and missing columns
-- This table was missing RLS policies which caused 42501 errors when trying to assign workouts
-- Also adding missing columns that the application expects
...
-- Athletes can view their own workout assignments
CREATE POLICY "Athletes can view their own workout assignments"
  ON public.athlete_workouts
  FOR SELECT
  USING (athlete_id = auth.uid());

-- Coaches can view workout assignments for their athletes (approved relationships only)
CREATE POLICY "Coaches can view their athletes workout assignments"
  ON public.athlete_workouts
  FOR SELECT
  USING (
    EXISTS (
...
-- Athletes can assign workouts to themselves (when creating their own workouts)
CREATE POLICY "Athletes can assign workouts to themselves"
  ON public.athlete_workouts
  FOR INSERT
  WITH CHECK (athlete_id = auth.uid());

-- Coaches can assign workouts to their athletes (approved relationships only)
...
-- Athletes can update their own workout assignment status (e.g., mark as completed)
CREATE POLICY "Athletes can update their own workout assignments"
  ON public.athlete_workouts
  FOR UPDATE
  USING (athlete_id = auth.uid())
  WITH CHECK (athlete_id = auth.uid());

-- Coaches can update workout assignments for their athletes (approved relationships only)
CREATE POLICY "Coaches can update their athletes workout assignments"
  ON public.athlete_workouts
  FOR UPDATE
  USING (
    EXISTS (
...
-- Athletes can delete their own workout assignments (if they want to remove a workout)
CREATE POLICY "Athletes can delete their own workout assignments"
  ON public.athlete_workouts
  FOR DELETE
  USING (athlete_id = auth.uid());

-- Coaches can delete workout assignments for their athletes (approved relationships only)
CREATE POLICY "Coaches can delete their athletes workout assignments"
  ON public.athlete_workouts
  FOR DELETE
  USING (
    EXISTS (
...
-- Create index for better performance on coach-athlete relationship lookups
