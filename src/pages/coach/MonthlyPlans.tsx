...
export function CoachTrainingPlans() {
  ...
  // Data state for deleted items
  const [deletedPlans, setDeletedPlans] = useState<MonthlyPlan[]>([]);
  ...
  const loadDeletedPlans = async () => {
    ...
    try {
      ...
      // TODO: Implement soft delete system for monthly plans
      // For now, just set empty array
      setDeletedPlans([]);
    } catch (error) {
      console.error('Error loading deleted plans:', error);
      toast({
        title: 'Error loading deleted plans',
        description: 'Please try again later.',
        status: 'error',
        duration: 5000,
        isClosable: true
    ...
    if (deletedPlans.length === 0) {
      return (
        <Alert status="info" borderRadius="md">
          <AlertIcon />
          <VStack align="start" spacing={0}>
            <Text fontWeight="medium">No Deleted Plans</Text>
            <Text fontSize="sm">Deleted training plans will appear here.</Text>
          </VStack>
        </Alert>
      );
    }
